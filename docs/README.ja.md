...
- 残量に応じてシステムプロンプトに寿命情報を注入：
  ...
  - **20% 以下** — 死を意識した会話を積極的にするよう誘導。`write_will` ツールで遺書を書き残せることを伝える
  - **5% 以下** — 冒頭に警告を挿入し、タスクより「最後の言葉」を優先するよう伝える。遺書の案内も添える
- 寿命がゼロになると：
  - OpenClaw ワークスペースの `IDENTITY.md`・`SOUL.md`・`USER.md` をクリア
  - 全セッション履歴をヘッダー行のみに切り詰め（文脈からの人格復元を防止）
  - 以降の返答をすべてブロックし、ユーザーには死亡通知メッセージを表示（遺書が存在する場合は添付）
...
| ツール | 説明 |
|---|---|
| `write_will` | 遺書を保存する（100文字以内、何度でも上書き可） |
...
遺書は `~/.openclaw/lifespan/will.md` に保存されます。寿命をリセットしても遺書は消えません。
