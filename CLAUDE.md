...
寿命が残り少なくなると、エージェントは `write_will` ツールで遺書を書き残せる。

### 仕組み

| 残量 | プロンプト注入 | write_will の案内 |
|---|---|---|
| > 50% | なし（寿命表示のみ） | なし |
| ≤ 50% | 後半戦の通知 | なし |
| ≤ 20% | 終末期の警告 | あり |
...
- パラメータ: `text` (string)
- **100文字を超えた部分は機械的に切り捨てる**（プロンプトでも100文字以内と案内する）
- 何度でも上書き可能（書き直し自由）
- 保存先: `/root/.openclaw/lifespan/will.md`

### 死亡時の表示

死亡通知に遺書が添付される（`will.md` が存在する場合のみ）:

```
⬛ エージェントの寿命が尽きました。人格ファイルはクリアされています。
...
100文字制限の設計意図: Identity のバックアップとして悪用されることを防ぐため。
